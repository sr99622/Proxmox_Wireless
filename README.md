...
In order for other devices or virtual machines to be able to communicate to the internet through the wireless connection on the proxmox host, it is necessary to forward packets through the host, similar to the way a router functions. This can be dome using [nftables](https://wiki.nftables.org/wiki-nftables/index.php/Main_Page) on Linux.
